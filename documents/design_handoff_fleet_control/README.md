...
- `screenshots/detail-view.png` — Agent detail page (hero + status, stat chips, current task with progress + step checklist, throughput chart, details panel).
- `screenshots/detail-view-log.png` — Agent detail, lower portion (step checklist with in-progress/pending states, metadata, color-coded streaming activity log).
- `screenshots/zen-mode.png` — Full-screen "zen mode" of the activity log (distraction-free: agent name + task header, Exit button, complete log in a roomy centered column).
- `screenshots/detail-carousel.png` — Detail view top bar showing the carousel control (Back · ‹ prev / position / next › · actions).
...
### Agent detail view (opened by clicking a card or table row)
Replaces the fleet view in place (same page; client state `selected`). Layout, top → bottom:
1. **Top bar** (space-between): **left group** = a pill **"← Back to fleet"** button, then a **carousel control** (a round ‹ prev button, a `current / total` position counter in mono, a round › next button); **right group** = action buttons: **RESPOND & RESUME** (coral, only when waiting), **Pause** (outline, only when working), **Stop** (outline, always).
2. **Hero** (space-between, wraps): left = status dot (12px, pulsing for working/waiting) + agent name (Space Grotesk 700, 40px) + status pill, with a mono sub-line `model · project · repo · agentId`; right = 4 stat chips (Elapsed, ETA, Progress [accent], Events) — each a `--panel` tile, 1px `--line`, radius 12px, mono label + 18px value.
3. **Body** — CSS grid `1fr 330px`, gap 18px, `align-items:start`.
   - **Main column** (gap 18px):
     - **Current task panel** (1px `--line`, radius 16px, padding 22px, gradient bg): "CURRENT TASK" label → task title (19px/600) → progress (step label + percent + 8px bar; shown when a task is active) → **step checklist** (one row per step: a 20px status circle [done = filled `--working` with `✓`; in-progress = `--working` ring, label "In progress" in accent; pending = `--line2` ring, muted], step name, right-aligned tag). Idle/offline show a muted "No active task" note instead.
     - **Activity log panel**: header is "ACTIVITY LOG" + pulsing green live dot, with an **Expand** button right-aligned (small outline pill, mono uppercase 10px, corners/expand icon; hover brightens text + border to accent). Below: a scrollable feed (`max-height` ~330px), newest first. Each line = time (mono, `--faint`) · kind tag (mono 9px, color-coded) · message (mono 12.5px). Kind colors: TOOL `--accent`, EDIT `#e8b06a`, RUN/DONE `#7ee0a6`, WAIT `--waiting`, INFO `--faint`.
   - **Sidebar** (gap 18px): **Throughput panel** (large area chart, `<svg viewBox="0 0 280 110">`, same fill/stroke logic) + **Details panel** (label/value rows: Model, Project, Repository, Agent ID, Started, Uptime).

Behavior: clicking a card or row sets `selected` (the RESPOND button on a waiting card also opens detail, via `stopPropagation`). Back clears it. **Pause** sets the agent to waiting, **Stop** sets it to idle (clears the task), **RESPOND & RESUME** resumes working — each appends a log line. The detail reads live from the selected agent, so progress, ETA, log, and throughput keep updating while open.

**Carousel navigation**: the detail view is a carousel over the full fleet (in list order, wrapping at both ends). The ‹ / › buttons and the **Left / Right arrow keys** move `selected` to the previous / next agent; the `current / total` counter reflects position. Navigation triggers a short directional slide (the hero + body slide in from the right when going next, from the left when going prev; ~0.34s ease) — implemented by toggling between two identical keyframes (`cvslidea`/`cvslideb`) to force restart, with the start offset set via a `--cvfrom` CSS variable (±38px). Opening a fresh card resets the offset to 0 (no slide). Arrow keys are only active while a detail is open; `Escape` is reserved for closing zen mode.

### Zen mode — full-screen activity log (opened by the Activity log panel's Expand button)
A distraction-free, full-screen view of the selected agent's complete activity log. Driven by a local boolean (`zen`); only meaningful while an agent is selected.
- **Overlay**: `position:fixed; inset:0`, above all app chrome (z-index ~90). Background = the app's warm-dark `--bg` plus a soft top radial glow (`radial-gradient(1200px 700px at 50% -10%, rgba(255,140,50,0.08), transparent 60%)`). Fades/slides in (`fadeup .3s`). Flex column: fixed header + scrollable body.
- **Header** (centered column, max-width ~920px, padding `38px 32px 22px`): status dot (10px, pulsing for working/waiting) · agent name (bold ~26px, -0.02em) · "ACTIVITY LOG" label (mono 10px, 0.13em, `--accent`) · pulsing green live dot + "live". A muted mono sub-line (~12px, `--faint`) shows the agent's current task. Top-right: an **Exit** pill (mono uppercase 11px, 1px `--line2`, corners-in icon; hover → text `--txt`, border `--accent`).
- **Body** (flex:1, `overflow:auto`): the full log in a centered column (max-width ~920px, padding `4px 32px 60px`), roomier than the panel — each row `padding:13px 0`, 1px `--line` divider: time (mono 13px, `--faint`, flex-basis 84px) · kind tag (mono 10px, 0.07em, color-coded, flex-basis 54px) · message (mono 15px, `--txt`, line-height 1.45). Newest first. No other chrome.
- **Behavior**: Expand sets `zen=true`; **Exit button OR the `Escape` key** sets it false; leaving the detail view (`Back`) also resets `zen`. The log keeps updating **live** while open (same data source as the panel — purely a presentation layer, no new data). Register the `keydown` listener on mount, remove on unmount.
...
- **Zen mode**: the Activity log panel's **Expand** button opens a full-screen, distraction-free view of the full log (see "Zen mode" above); **Exit** or **Esc** closes it; it stays live while open.
- **Detail carousel**: on the detail view, the ‹ / › buttons and **← / → arrow keys** navigate to the previous / next agent (wraps around), with a short directional slide; a `current / total` counter shows position.

## State Management
Prototype state (replace with your store + live data):
- `view`: 'cards' | 'table'
- `filter`: 'all' | 'working' | 'waiting' | 'idle' | 'offline'
- `selected`: agent name (string) | null — drives the detail view (null = fleet view)
- `zen`: boolean — full-screen activity-log overlay for the selected agent (reset to false when `selected` clears)
- `navParity` (0/1) + `navFrom` (CSS length) — drive the carousel slide animation (parity toggles to restart the keyframe; `navFrom` is the ± start offset, `'0px'` on fresh open)
- `live`: boolean (timer on/off)
- `agents[]`: each `{ name, model, project, repo, agentId, state, task, step, total, progress, eta, elapsed, events, stepNames[], hist[], log[] }`
  - `hist` is a ring buffer (length 28) of normalized 0–1 activity values that feeds the sparkline.
  - `stepNames` is the ordered checklist for the current task; `log` is a ring buffer (≤60) of `{ ts, kind, msg }` activity entries.
...
- `screenshots/` — cards-view-top.png, cards-view.png, table-view.png, detail-view.png, detail-view-log.png, zen-mode.png, detail-carousel.png.
